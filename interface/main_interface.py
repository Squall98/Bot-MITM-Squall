    ...
    def character_statue(self):
        ...
        while True:
            ...
            if self.character.deplacement.ismouving:
                en_mouvement.after(0, lambda: en_mouvement.configure(bg="Green"))
            else:
                en_mouvement.after(0, lambda: en_mouvement.configure(bg="Red"))
            
            if self.character.isharvest:
                en_recolte.after(0, lambda: en_recolte.configure(bg="Green"))
            else:
                en_recolte.after(0, lambda: en_recolte.configure(bg="Red"))
            
            if self.character.isfighting:
                en_combat.after(0, lambda: en_combat.configure(bg="Green"))
            else:
                en_combat.after(0, lambda: en_combat.configure(bg="Red"))
